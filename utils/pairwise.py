#!/usr/bin/env python3
"""
Legacy pairwise scan-comparison engine.

Compares two scan-result JSON payloads field-by-field (find_matches /
compare_pair / MATCH_WEIGHTS) and ranks a directory of scans pairwise. This is
the *older* correlation path: the live web app scores linkage over the global
attribution graph instead (utils/check.py). Nothing in the request path imports
this module — it is used only by core/basic.py's CLI batch/dive mode and the
standalone command below. Kept separate so utils/check.py contains only the
engine the app actually runs.

Single-pair mode:
    python -m utils.pairwise <a.json> <b.json> <out.json>

Batch mode — pairwise compare every JSON in a directory:
    python -m utils.pairwise --dir <scans_dir> <out_dir>
"""
    ...
    if isinstance(a, dict):
        ...
        if fields:
            for field in fields:
                ...
                # These fields are scalars (fingerprints, IPs, ids), so set
                # membership makes the intersection O(1) per value instead of a
                # linear scan of b_vals (and of the growing `shared` list).
                b_vals = set(_extract_field(b, field))
                shared: list = []
                seen: set = set()
                for v in a_vals:
                    if v in b_vals and v not in seen:
...
def assess_cert_match_quality(a: dict, b: dict) -> dict:
    ...
    # Require the cert to actually name BOTH domains, not just one. A cert is
    # trivially "relevant" to whichever domain it was scanned from — a's own
    # server will almost always present a cert naming a, wildcard or not — so
    # an `or` here passed on nothing more than "domain A has a normal cert."
    # The real question is whether that identical certificate is *also* a's
    # or b's own cert, or whether one side is just an edge/shared box handing
    # out the other side's (possibly wildcard) cert as its default vhost —
    # a false positive for "shared operator" that instead means "shared box".
    relevant = (_cert_is_relevant(cert_a, a_domain)
                and _cert_is_relevant(cert_b, b_domain))
